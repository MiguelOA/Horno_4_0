// Pendientes:
// 1.- Incluir el adquir temperatura y grabarla en la base de datos
...
io.on('connection', (socket) => {
	...
	socket.on('alta', (data6)=>{
		...
		let command = 'INSERT INTO bitacora (usuario, fecha, t_inicio, t_final, equipo, sp) ';
...
function revision_estados(){
	...
	if (lecHardware){
		...
		exec("./I2C_SQL D datos estado 1; ./I2C_SQL 0 datos temp 1", (error, stdout, stderr) => {
			if (error) {
				if (error.code != 1){
					notifyRegister(fileReg,'Error de lectura de estado por I2C');
				}
				return;
			}
			if (stderr) {
...
function nuevaEntrada(){
	...
	let Hoy = dia_a + '/';
	let id_a;
	let minTotal = 0; //********* Cambiar la hora a minutos del dia para comparar con la DB
	minTotal = parseInt(hora_a) * 60 + parseInt(minutos_a);
	//Comparativo de fechas entre hoy y DB en texto
	...
	dataBase.query(command2, function (err, result, fields){
		if (result.length != 0) {
			...
			let horaAT = result[0].t_inicio;
			let horaA = horaAT.split(":");
			let minA = parseInt(horaA[0]) * 60 + parseInt(horaA[1]);
			if (Hoy == result[0].fecha && minTotal >= minA){  //poner la restriccion del horario
